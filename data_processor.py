...
from data_fetcher import get_batch
import requests
...
def get_pos_dataset(search_url: str, output_base_name: str, session: requests.Session) -> None:
    ...
    tsv_file_name = os.path.join(DATA_DIR, f"{output_base_name}.tsv")
    fasta_file_name = os.path.join(DATA_DIR, f"{output_base_name}.fasta")
